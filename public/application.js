...
var gameOver = false;
var numTurns = 0;
var addtoGameIndex = void 0;
var addtoGameValue = void 0;
var addingGameValueFunction = void 0;

// pushing x and o
var createx = function createx(index) {
  game[index] = 'x';
};

var createo = function createo(index) {
  game[index] = 'o';
};
...
var placeLetter = function placeLetter(event) {
  event.preventDefault(); // preventing page refresh
  if ($('.userMessage').text() !== 'X goes first so click a square to begin' && $('.userMessage').text() !== '' && $('.userMessage').text() !== 'X WINS!' && $('.userMessage').text() !== 'O WINS!' && $('.userMessage').text() !== 'This game is over. Click the button below to start a new game or Sign Out' && $('.userMessage').text() !== 'Tic Tac Tie!') {
  ...
  if (gameOver) {
    // whether clicks should be firing -- should not if game is over
  ...
  if (whoseTurn() === 'x') {
    // x on 0 and evens and o on odds
...
var addToGame = function addToGame(data) {
  api.addingMoves(data).then(ui.addToGameSuccess).catch(ui.addToGameFailure);
};
...
var checkForWin = function checkForWin() {
  if (game[0] === 'x' && game[1] === 'x' && game[2] === 'x' || game[3] === 'x' && game[4] === 'x' && game[5] === 'x' || game[6] === 'x' && game[7] === 'x' && game[8] === 'x' || game[0] === 'x' && game[4] === 'x' && game[8] === 'x' || game[0] === 'x' && game[3] === 'x' && game[6] === 'x' || game[1] === 'x' && game[4] === 'x' && game[7] === 'x' || game[2] === 'x' && game[5] === 'x' && game[8] === 'x' || game[2] === 'x' && game[4] === 'x' && game[6] === 'x') {
    $('.userMessage').text('X WINS!');
    gameOver = true;
    return;
  } else if (game[0] === 'o' && game[1] === 'o' && game[2] === 'o' || game[3] === 'o' && game[4] === 'o' && game[5] === 'o' || game[6] === 'o' && game[7] === 'o' && game[8] === 'o' || game[0] === 'o' && game[4] === 'o' && game[8] === 'o' || game[0] === 'o' && game[3] === 'o' && game[6] === 'o' || game[1] === 'o' && game[4] === 'o' && game[7] === 'o' || game[2] === 'o' && game[5] === 'o' && game[8] === 'o' || game[2] === 'o' && game[4] === 'o' && game[6] === 'o') {
...
var clearGameArray = function clearGameArray() {
  game = ['', '', '', '', '', '', '', '', ''];
};

var clearNumTurns = function clearNumTurns() {
  numTurns = 0;
};
var newGame = function newGame(event) {
  event.preventDefault();
  if ($('.userMessage').text() !== 'X goes first so click a square to begin' && $('.userMessage').text() !== '' && $('.userMessage').text() !== 'X WINS!' && $('.userMessage').text() !== 'O WINS!' && $('.userMessage').text() !== 'This game is over. Click the button above to start a new game or Sign Out' && $('.userMessage').text() !== 'Tic Tac Tie!') {
    return; // need to be signed in to play -- looks at messages to determine to make global
  }
  clearGameArray();
  clearBoard();
...
var onGameUpdate = function onGameUpdate(index, value, over) {
  var data = {
    'game': {
      'cell': {
        'index': index,
...
module.exports = {
  ...
  clearGameArray: clearGameArray,
  clearNumTurns: clearNumTurns,
  numTurns: numTurns,
  whoseTurn: whoseTurn,
  gameOver: gameOver,
  addingGameValueFunction: addingGameValueFunction,
  addToGame: addToGame
};
...
var addToGameSuccess = function addToGameSuccess(data) {
  store.game = data;
};

var addToGameFailure = function addToGameFailure() {};

var showAllGamesSuccess = function showAllGamesSuccess(data) {
  $('#numGames-message').show();
  $('#numGames-message').text('You have played ' + data.games.length + ' games!');
};

var showAllGamesFailure = function showAllGamesFailure() {};
...
module.exports = {
  ...
  addToGameSuccess: addToGameSuccess,
  addToGameFailure: addToGameFailure,
  showAllGamesSuccess: showAllGamesSuccess,
...
var addingMoves = function addingMoves(data) {
  return $.ajax({
...
var createGame = function createGame(event) {
  ...
  var data = gameEngine.game;
  api.newGame(data).then(ui.createGameSuccess).catch(ui.createGameFailure);
};

var allGames = function allGames(event) {
  event.preventDefault();
  $('#password-change-message').hide();
  $('#successMessage').text('');
  api.showAllGames().then(ui.showAllGamesSuccess).catch(ui.showAllGamesFailure);
};
